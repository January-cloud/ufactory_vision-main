...
CAND_BIN_MM = 8        # 物块聚类网格(mm)，要比"单物块抖动"大、比"物块间距"小
CAND_MIN_FRAMES = 6    # 主簇至少占多少帧才算锁定(占比越高越稳)
...
def main():
    ...
    # ---------- 初始化GGCNN(OPEN_LOOP_HEIGHT=0→始终取全局最大值) ----------
    ...
    off_r = max(0, CAM_HEIGHT - cs) // 2
    off_c = max(0, CAM_WIDTH - cs) // 2
    cand_buf = []            # 候选目标缓冲(多物体聚类用)
    grasp_count = 0          # 抓取计数
    last_time = 0            # 上次抓取时间(冷却用)

    # ---------- 主循环 ----------
    while arm.connected:
        # ② 报错自动恢复：清错→复位→回观察位→继续
        if arm.error_code != 0:
            recover(arm)
            cand_buf.clear()
        ...
        if result is not None and result[2] > MIN_RESULT_Z_MM / 1000.0:
            cand = cam_result_to_base(eef, result)           # 本帧候选(base坐标)
            cand_buf.append(cand)                            # 存入候选缓冲
            if len(cand_buf) > CAND_WINDOW: cand_buf.pop(0)  # 保持窗口长度
            if len(cand_buf) >= CAND_WINDOW:
                pick, cnt = dominant_cluster(cand_buf, CAND_BIN_MM)   # 选最稳的一个物块
                if cnt >= CAND_MIN_FRAMES:                   # 该物块被稳定识别→锁定
                    stable = True
                    goal = pick
                else:
                    goal = cand                              # 未锁定，仅显示用
            ...
            cv2.circle(color_crop, (int(mp[1]), int(mp[0])), 6,
                       (0,255,0) if stable else (0,200,255), -1)   # 绿=锁定/橙=搜索
        else:
            cand_buf.clear()                                 # 没识别到→清空缓冲
        ...
        if stable and in_range(goal) and (time.monotonic() - last_time) > COOLDOWN_SEC:
            ...
            last_time = time.monotonic(); cand_buf.clear()   # 抓完清空，下一轮找下一个物块
